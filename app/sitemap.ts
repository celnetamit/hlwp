...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const baseRoutes: MetadataRoute.Sitemap = [
    ...
    {
      url: `${siteUrl}/about`,
      lastModified: new Date('2024-01-01'), // Static page
  ...
  const journalRoutes: MetadataRoute.Sitemap = journals.map((journal: any) => {
    const lastModified = journal.lastUpdated 
  ...
  const categories = [...new Set(
    journals
      .map((journal: any) => journal.category)
      .filter(Boolean)
  )]
  
  const categoryRoutes: MetadataRoute.Sitemap = categories.map((category: string) => {
    const categoryJournals = journals.filter((journal: any) => journal.category === category)
    const mostRecentUpdate = categoryJournals
      .map((journal: any) => journal.lastUpdated ? new Date(journal.lastUpdated) : new Date())
      .sort((a: Date, b: Date) => b.getTime() - a.getTime())[0] || currentDate

    return {
      url: `${siteUrl}/category/${encodeURIComponent(category.toLowerCase().replace(/\s+/g, '-'))}`,
      lastModified: mostRecentUpdate,
      changeFrequency: 'daily' as const,
      priority: Math.min(0.8, 0.5 + (categoryJournals.length * 0.01)), // Higher priority for categories with more journals
    }
  })
  ...
  const subjects = [...new Set(
    journals
      .flatMap((journal: any) => journal.subjects || journal.keywords || [])
      .filter(Boolean)
  )]
  
  const subjectRoutes: MetadataRoute.Sitemap = subjects.slice(0, 100).map((subject: string) => { // Limit to top 100 subjects
    const subjectJournals = journals.filter((journal: any) => 
      (journal.subjects || []).includes(subject) || 
      (journal.keywords || []).includes(subject)
    )

    return {
      url: `${siteUrl}/subject/${encodeURIComponent(subject.toLowerCase().replace(/\s+/g, '-'))}`,
      lastModified: currentDate,
      changeFrequency: 'weekly' as const,
  ...
  const publishers = [...new Set(
    journals
      .map((journal: any) => journal.publisher)
      .filter(Boolean)
  )]
  
  const publisherRoutes: MetadataRoute.Sitemap = publishers.map((publisher: string) => {
    const publisherJournals = journals.filter((journal: any) => journal.publisher === publisher)

    return {
      url: `${siteUrl}/publisher/${encodeURIComponent(publisher.toLowerCase().replace(/\s+/g, '-'))}`,
      lastModified: currentDate,
  ...
  const articleRoutes: MetadataRoute.Sitemap = journals
    .flatMap((journal: any) => (journal.articles || []).slice(0, 10)) // Limit to 10 most recent articles per journal
    .filter(Boolean)
    .map((article: any) => ({
